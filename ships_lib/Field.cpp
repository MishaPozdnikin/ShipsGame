...
void Field::initialize(const char& c) {
  for (int i = 0; i <= height + 1; i++) {
    for (int j = 0; j <= width + 1; j++) {
      ...
      if (arr[i][j] != border_sign) this->arr[i][j] = c;
    }
  }
}
...
void Field::spawn_ships(const int& ship_deck_number, const int& ships_number) {
  if (ships_number * ship_deck_number > height * width) {
    std::cout << "too much ships for this field. max ships number is "
              << height * width << "\n";
    return;
  }

  int counter = 0, x = 0, y = 0, direction = 0, temp_x = x, temp_y = y,
      check = 0;
  ...
  while (counter < ships_number) {
    ...
    for (int i = 0; i < ship_deck_number; i++) {
      ...
      if (arr[y][x] == ship_sign || arr[y][x + 1] == ship_sign ||
          arr[y][x - 1] == ship_sign || arr[y + 1][x] == ship_sign ||
          arr[y - 1][x] == ship_sign || arr[y + 1][x - 1] == ship_sign ||
          arr[y + 1][x + 1] == ship_sign || arr[y - 1][x - 1] == ship_sign ||
...
bool Field::is_ship(const int& x, const int& y) {
  if (arr[y][x] == ship_sign || arr[y][x] == destroyed_sign) return true;
  return false;
}
...
void Field::draw_ship_perimetr_with_tried_sign(Ship* sh) {
  ...
  for (int i = 0; i < sh->get_deck_number(); i++) {
    ...
    if (arr[y + 1][x] != border_sign && arr[y + 1][x] != ship_sign &&
        arr[y + 1][x] != destroyed_sign && arr[y + 1][x] != tried_sign) {
    ...
    if (arr[y][x + 1] != border_sign && arr[y][x + 1] != ship_sign &&
        arr[y][x + 1] != destroyed_sign && arr[y][x + 1] != tried_sign) {
    ...
    if (arr[y + 1][x + 1] != border_sign && arr[y + 1][x + 1] != ship_sign &&
...
int Field::get_ships_num() { return curr_ships_num; }
...
bool Field::is_destroyed(const int& x, const int& y) {
  if (get_ship(x, y).get_is_destroyed()) return true;
  return false;
}
...
void Field::try_point(const int& x, const int& y) { arr[y][x] = tried_sign; }
