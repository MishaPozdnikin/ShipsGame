...
GAME_MODE
Game::getGameMode() { return this->game_mode; }
...
CURRENT_PLAYER
Game::getCurrentPlayer() { return this->current_player; }
...
void Game::enemy_shoot(Field* my_field) {
  ...
  while (1) {
    ...
    if (my_x < my_field->get_width() + 1 && my_y < my_field->get_height() + 1 &&
        my_x > 0 && my_y > 0 && !my_field->is_destroyed_sign(my_x, my_y) &&
        !my_field->is_tried(my_x, my_y)) {
      std::cout << "\nEnemy tries: x = " << my_x << " y = " << my_y << ".\n\n";
      if (my_field->is_ship(my_x, my_y)) {
        my_field->destroy_ship(my_x, my_y);
        std::cout << "Your ship has been destroyed at x " << my_x << " y "
                  << my_y << ".\n";
...
void Game::player_shoot(Field* enemy_field, Field* my_field) {
  ...
  while (1) {
    ...
    if (game_mode == GAME_MODE::CvC) {
      std::cout << "Computer " << getCurrentPlayer() - 1
                << ". Your field:" << std::endl
                << std::endl;
    } else {
      std::cout << "Player " << getCurrentPlayer() + 1
                << ". Your field:" << std::endl
                << std::endl;
    }
    ...
    if (game_mode != GAME_MODE::CvC) {
      ...
      while (1) {
        ...
        if (enemy_x < enemy_field->get_width() + 1 &&
            enemy_y < enemy_field->get_height() + 1 && enemy_x > 0 &&
            enemy_y > 0 && !enemy_field->is_destroyed_sign(enemy_x, enemy_y) &&
            !enemy_field->is_tried(enemy_x, enemy_y)) {
          Sleep(1000);
    ...
    if (enemy_x < enemy_field->get_width() + 1 &&
        enemy_y < enemy_field->get_height() + 1 && enemy_x > 0 && enemy_y > 0 &&
        !enemy_field->is_destroyed_sign(enemy_x, enemy_y) &&
        !enemy_field->is_tried(enemy_x, enemy_y)) {
